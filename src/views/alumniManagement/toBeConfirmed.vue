		...
		<base-layout>
			<template slot="header">
				<el-form ref="params" :model="params" label-width="6em" class="demo_ruleForm">
					<el-row>
						<el-col :span="4">
							<el-form-item label="确认情况" prop="classifyTemp">
								<el-select @clear="search" clearable v-model="params.classifyTemp" class="w-100" placeholder="请选择确认情况">
									<el-option v-for="item in confirmStatusList" :key="item.label + item.value" :label="item.label"
										:value="item.value" />
								</el-select>
							</el-form-item>
						</el-col>
			...
			<template slot="main">
				<div class="contorl_container">
					...
					<div class="contorl_btns">
						...
						<el-button type="text" @click="showPendingDialog(multipleSelection)">待定</el-button>
						<el-button type="text" @click="showDerive(multipleSelection, '0')">导出</el-button>
					</div>
				</div>
				<!-- 表格列表 -->
				<Etable v-if="!isGrid" ref="tableRef" @sortChange="sortChange" @selectLine="handleSelectionChange" selection height="100%" :table-head-config="tableHeadConfig" :table-load-data="tableData"
					...
					<template slot="classifyTemp" slot-scope="{ data }">
						<div>
							{{ classifyStatusMap[data.row.classify] || defaultSettings.defaultTableEmptyText }}
						</div>
					</template>
				...
				<div class="card_list_container" v-else>
					<alumniCardList :selectedList="multipleSelection" @select="handleSelectionChange" type="1" :list="tableData" @confirm="showConfirmDialog" @detail="showDetail" @delete="showDel" />
				</div>
			</template>
...
import { pendingTableOption } from './data/tableOption'
...
export default {
	...
	data() {
		return {
			...
			generList,
			confirmStatusList,
			tableHeadConfig: pendingTableOption,
			classifyStatusMap: {
				'0': '未定',
				'1': '待定'
			},
	...
	methods: {
		...
		showDetail(row) {
			...
			// 	path: '/alumniManagement/toBeConfirmed/detail',
			// 	query: {
			// 		id
			// 	}
			// })
